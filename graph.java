...
public class sol{
    ...
    public static void addvertex(int x){
        for(Graph g:all)if(g.data==x)return;
    ...
    public static void printadjtable(){
        System.out.println("Adjacency is");
        for(Graph te:all){
    ...
    public static void pdfs(int x){
        Graph so=new Graph(0);
        for(Graph te:all)if(te.data==x)so=te;
    ...
    public static void bfs(Graph s){
        ...
        while(!q.isEmpty()){
            ...
            if(temp.Neighbours.size()!=0){
                ...
                for(Graph g:temp.Neighbours){if(!g.vis){q.add(g);g.vis=true;}}
            }
        }
